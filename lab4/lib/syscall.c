...
void sleep(int time){
    syscall(SYS_sleep,time,1,1,0,0);
}
...
int createSem(sem_t* sem, int value){
    return syscall(SYS_createSem,(uint32_t)sem,value,1,0,0);
}

int destroySem(sem_t *sem){
    return syscall(SYS_destroySem,(uint32_t)sem,1,1,0,0);
}
int lockSem(sem_t *sem){
    return syscall(SYS_lockSem, (uint32_t)sem, 1, 1, 0, 0);
}

int unlockSem(sem_t *sem){
    return syscall(SYS_unlockSem, (uint32_t)sem, 1, 1, 0, 0);
}
